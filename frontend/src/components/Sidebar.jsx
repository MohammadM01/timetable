...
const Sidebar = () => {
  ...
  const { principal } = useContext(SchoolContext);
      ...
      <div className="flex flex-col h-full">
        ...
        <div className="p-4 border-t border-slate-200">
          <div className="flex items-center rounded-2xl border border-slate-200 bg-white p-3 shadow-sm">
            ...
            {!isCollapsed && (
              <div className="ml-3">
                <p className="text-sm font-black text-slate-800 leading-tight" title={principal?.name || "Principal"}>
                  {principal?.name || "Principal"}
                </p>
                <p className="text-xs font-bold text-slate-500 mt-0.5">Constraint Admin</p>
              </div>
            )}
          </div>
        </div>
